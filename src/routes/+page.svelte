...
<div class="flex flex-col items-center gap-8">
  ...
  {:else}
    ...
    <div class="flex gap-4">
      ...
      <a
        href={`?page=${pageNumber}`}
        class={`rounded p-2 bg-blue-500 text-white font-bold disabled:opacity-50 flex justify-center items-center gap-2 ${books.length <= limit ? 'opacity-50 pointer-events-none' : ''}`}
        on:click={() => pageNumber = pageNumber + 1}
      >
        <span>Next</span>
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="w-5 h-5">
...
<!-- <div class="flex flex-col items-center gap-8">
  ...
  <div class="flex gap-4">
    ...
    <p class="flex items-center">{page} / {data.books.length}</p>
    <button
      type="button"
      class="rounded p-2 bg-blue-500 text-white font-bold disabled:opacity-50"
      on:click={() => page = page + 1}
  ...
  <div class="flex gap-2">
    ...
    />
    <p class="p-2">/ {data.books.length}</p>
    <button
      type="button"
